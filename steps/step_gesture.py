...
def step_impl(context):
    ...
    if diff_data > 1.0:
        ...
        when < 结束倒车
        ''')
        raise Exception('车机不在倒车界面,请参考截图信息: http://' + Utils().get_conf_value('socketIp') + ':9000/' + time.strftime('%Y%m%d') + '/screenshots/' + error_png)
...
def step_impl(context):
    ...
    if not diff_data > 1000.0:
        ...
        context.execute_steps('''
            when < 结束倒车
            ''')
        raise Exception('车机在倒车界面,请参考截图信息: http://' + Utils().get_conf_value('socketIp') + ':9000/' + time.strftime('%Y%m%d') + '/screenshots/' + error_png)
