...
class Utils:
    ...
    def raise_Exception_info(self, err_msg):
        png_name = self.take_screenshot()
        if self.crash_handler():
            raise Exception('应用crash，' + err_msg + '请参考截图信息: http://' + self.get_conf_value('socketIp') + ':9000/' + time.strftime('%Y%m%d') + '/screenshots/' + png_name)
            # + ' 和:  http://10.10.99.87:9000/' + time.strftime('%Y%m%d') + '/' + sce_name + '.log' + ' 对应场景日志信息 ')
        else:
            raise Exception('用例运行失败，' + err_msg + '请参考截图信息: http://' + self.get_conf_value('socketIp') + ':9000/' + time.strftime('%Y%m%d') + '/screenshots/' + png_name)
            # + ' 和:  http://10.10.99.87:9000/' + time.strftime('%Y%m%d') + '/' + sce_name + '.log' + ' 对应场景日志信息 ')
